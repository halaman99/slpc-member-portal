...
if (!fs.existsSync(adminDir)) {
  ...
  console.log('Created admin directory:', adminDir);
}
...
export default function AdminDashboard() {
  ...
  // Fetch admin metrics and audit log
  useEffect(() => {
  ...
  const isOverallLoading =
    authLoading || membersLoading || invitesLoading || loading
    ...
    <AdminLayout user={member}>
      ...
      <div className="mb-8">
        ...
        <p className="text-[#8a6070] text-sm">
          Manage your community and oversee all portal activities
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard
          ...
          badge={{
            text: members.length > 0 ? 'Growing' : 'Low',
            color: members.length > 0 ? 'green' : 'amber',
          }}
        />
        ...
        <StatCard
          ...
          sub="Admin capacity"
          badge={{
            text: adminCount < 5 ? 'Available' : 'Full',
            color: adminCount < 5 ? 'green' : 'amber',
          }}
        />
        ...
        <StatCard
          ...
          badge={{
            text: pendingRequests.length > 0 ? 'Action needed' : 'Clear',
        ...
        <StatCard
          ...
          badge={{
            text: systemHealth > 80 ? 'Excellent' : 'Good',
            color: systemHealth > 80 ? 'green' : 'amber',
          }}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <Card>
          ...
          <div className="space-y-2">
            <button className="w-full text-left px-4 py-3 rounded-lg bg-[#2a1010] hover:bg-[#3a1a1a] border border-[rgba(201,162,39,0.2)] text-sm text-[#c9a227] transition-colors font-medium">
              ➕ Create Member
            </button>
            <button className="w-full text-left px-4 py-3 rounded-lg bg-[#2a1010] hover:bg-[#3a1a1a] border border-[rgba(201,162,39,0.2)] text-sm text-[#c9a227] transition-colors font-medium">
              🔗 Send Admin Invite
          ...
          <div className="space-y-2">
            {admins.length > 0 ? (
              admins.slice(0, 4).map((admin, idx) => (
                <div
                  key={admin.id || idx}
                  className="flex items-center justify-between py-2 px-3 rounded-lg bg-[#1a0808] border border-[rgba(201,162,39,0.05)] hover:border-[rgba(201,162,39,0.2)] transition-colors"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-6 h-6 rounded-full bg-[#2a1010] border border-[#c9a227] flex items-center justify-center text-xs font-bold text-[#c9a227] flex-shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[#6a5060] truncate">
                        {admin.email}
                      </p>
                    </div>
                  </div>
              ...
              <div className="py-4 text-center text-[#6a5060] text-xs italic">
                No admins found
              </div>
            )}
            {admins.length > 4 && (
              <p className="text-xs text-[#6a5060] text-center pt-2">
        ...
        <Card>
          <CardHeader
            ...
            link={{ label: pendingRequests.length.toString(), href: '#' }}
          />
          <div className="space-y-2">
              ...
              pendingRequests.slice(0, 4).map((req, idx) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-[#6a5060]">
                      Sent{' '}
      ...
      <Card>
        <CardHeader
          ...
          icon={<Activity className="w-4 h-4 text-[#c9a227]" />}
        />
        <div className="space-y-1">
          {auditLog.length > 0 ? (
            auditLog.slice(0, 10).map((log, idx) => (
              <div
                key={log.id || idx}
                className="flex items-start gap-3 py-3 px-3 border-b border-[rgba(255,255,255,0.05)] last:border-0 hover:bg-[#1a0808] transition-colors rounded"
              >
                <div className="w-2 h-2 rounded-full bg-[#c9a227] mt-1.5 flex-shrink-0"></div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#e8dcc8] capitalize">
                    {log.action}
                  </p>
                  <div className="flex items-center justify-between gap-2 mt-1">
                    <p className="text-xs text-[#6a5060] truncate">
                      {log.details?.description ||
                        log.details?.member_name ||
                        'System action'}
                    </p>
                    <span className="text-[10px] text-[#5a4050] whitespace-nowrap">
...
fs.writeFileSync(path.join(adminDir, 'page.tsx'), pageContent);
console.log('Created page.tsx');

// Create layout.tsx
const layoutContent = `// Admin section layout
...
console.log('✓ Admin directory structure created successfully');
